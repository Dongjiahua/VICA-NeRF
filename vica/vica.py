...
@dataclass
class VICAModelConfig(NerfactoModelConfig):
    """Configuration for the VICAModel."""
    _target: Type = field(default_factory=lambda: VICAModel)
...
class VICAModel(NerfactoModel):
    """Model for VICA."""

    config: VICAModelConfig
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None, depth_gt=None):
        loss_dict = {}
        ...
        if depth_gt is not None:
            loss_dict["depth_loss"] = self.rgb_loss(depth_gt, outputs["depth"])
        if self.training:
